...
function Collection(props) {
  ...
  const handleNoteSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `https://vast-ravine-96250.herokuapp.com/api/notes`, newNote, token
      );
      setNotes([data, ...notes]);
  ...
  useEffect(() => {
    const getCollection = async () => {
      try {
        const { data } = await axios.get(
          `https://vast-ravine-96250.herokuapp.com/api/collections/${props.match.params.id}`
        );
        setCollection(data);
    ...
    const getNotes = async () => {
      try {
        const { data } = await axios.get(
          `https://vast-ravine-96250.herokuapp.com/api/collections/${props.match.params.id}/notes`, token
        );
        setNotes(data);
      } catch (err) {
        console.log(err);
  ...
  const handleScanUpload = async (e) => {
    ...
    if (selectedFile) {
      ...
      try {
        ...
        const { data } = await axios.post('https://vast-ravine-96250.herokuapp.com/api/upload', formData, config);
        setScan(data.scan);
      } catch (err) {
        console.log(err);
      }
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm('Are you sure you want to delete this?')) {
      try {
        await axios.delete(`https://vast-ravine-96250.herokuapp.com/api/collections/${id}`);
